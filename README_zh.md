...
### 2. **按学习路径进行**
本仓库按循序渐进的学习旅程组织，从基础到高级：

| 阶段 | 项目编号 | 重点 | 获得的技能 |
|-------|----------|-------|--------------|
| 初级 | 001-002 | 环境搭建，FreeRTOS基础 | 开发环境设置，基本概念 |
| 中级 | 003-006 | 任务管理，内存模型 | 任务创建/删除，内存优化 |
| 高级 | 007-012 | 任务间通信与同步 | 参数传递，队列，信号量，资源管理 |

### 3. **探索示例项目**
每个示例都在前一个基础上构建，逐步增加复杂度和功能：

| 编号 | 示例项目 | 描述 | 关键技能 |
|-------|----------|-------|--------------|
| 001 | STM32项目模板 | STM32开发的起点 | 标准外设库配置 |
| 002 | FreeRTOS基础示例 | FreeRTOS的第一步 | 任务创建与运行 |
| 003 | 动态任务创建 | 多任务处理基础 | 动态内存分配 |
| 004 | 静态任务创建 | 内存优化的任务创建 | 静态内存分配 |
| 005 | 项目完善 | 添加串口通信、调试功能 | 外设集成与调试技术 |
| 006 | 多任务操作 | 创建、删除和监控任务 | 任务生命周期管理 |
| 007 | 任务参数 | 使用参数实现灵活的任务函数 | 参数传递技术 |
| 008 | 队列通信 | 基本的任务间通信 | 数据共享与传输 |
| 009 | 队列高级操作 | 高效的数据传输技术 | 结构化数据传输 |
| 010 | 二值信号量 | 任务同步与互斥访问控制 | 资源保护与同步 |
| 011 | 计数型信号量 | 资源管理与事件计数 | 资源池管理 |
| 012 | 优先级翻转 | 理解和解决优先级翻转问题 | 系统稳定性与实时性 |
...
- **[012]** [FreeRTOS优先级翻转详解与实战教程](docs/zh/012-FreeRTOS优先级翻转详解与实战教程.md)
...
#### [004] 静态任务创建
探索任务创建的静态内存分配 - 对于内存受限的嵌入式系统至关重要。
...
#### [010] 二值信号量
掌握二值信号量的使用，实现任务同步和互斥资源访问。

#### [011] 计数型信号量
学习使用计数型信号量进行资源池管理和事件计数，通过实例掌握高级同步场景的应用。

#### [012] 优先级翻转
学习认识实时系统中的优先级翻转问题，理解其产生原因，并掌握如何通过优先级继承和互斥量等技术解决这一棘手问题。
